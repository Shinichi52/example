export function formatNumberNew2(input1, decimal, fill) {
    try {
        let input = input1;
        if (decimal == null) {
...
export function roundFloat(numberFloat, lenght) {
    try {
        ...
        arrNumber = arrNumber.replace(/^(-?)/, `$10`.repeat(lenght))
        const result = Number(`${arrNumber.substring(0, arrNumber.length - lenght)}.${arrNumber.substr(-lenght)}`);
        return result
    } catch (e) {
        console.error(e);
    }
